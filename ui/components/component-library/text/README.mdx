...
Use the `color` prop and the `COLOR` object from `./ui/helpers/constants/design-system.js` to change the color of the `Text` component.
...
<Text color={COLORS.SECONDARY_DEFAULT}>
  secondary-default
</Text>
<Text color={COLORS.SECONDARY_INVERSE} backgroundColor:{COLORS.SECONDARY_DEFAULT}>
  secondary-inverse
</Text>
...
Use the `margin` prop to change margin of the `Text` component. For more control over bounding box properties use the `boxProps` props object.

<Canvas>
  <Story id="ui-components-component-library-text-text-stories-js--margin" />
</Canvas>
...
<Text margin={4}>This uses the boxProps prop</Text>;
```
